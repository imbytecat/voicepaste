...
export function Overlay() {
        ...
        : phase === "starting"
          ...
          : phase === "success"
            ...
            : phase === "error"
              ...
              : "VoicePaste";
  const displayText =
    phase === "finishing" ? "正在生成最终文本…" : text || (phase === "recording" ? "请开始说话…" : "按快捷键开始听写");
  const orbColor =
    phase === "success"
  ...
  const statusColor = phase === "success" ? "text-[#86efac]" : phase === "error" ? "text-[#fda4af]" : "text-[#b8afff]";
      ...
      <div className="grid h-full w-full animate-in grid-cols-[40px_minmax(0,1fr)_74px] items-center gap-2.5 rounded-[28px] border border-white/13 bg-[linear-gradient(110deg,rgba(27,29,38,0.96),rgba(11,12,17,0.97))] py-2 pr-3.5 pl-2.5 shadow-[0_14px_38px_rgba(0,0,0,0.34),inset_0_1px_0_rgba(255,255,255,0.05)] backdrop-blur-3xl duration-200 zoom-in-95 fade-in slide-in-from-bottom-2">
        ...
        >
          {phase === "starting" || phase === "finishing" ? (
            <LoaderCircle className="size-5 animate-spin text-white" strokeWidth={2.2} />
          ) : phase === "success" ? (
        ...
        <div className="min-w-0" aria-live="polite">
          <div className="mb-0.5 flex items-center gap-2 overflow-hidden whitespace-nowrap">
            <span className={`shrink-0 text-[9px] font-bold tracking-[0.08em] ${statusColor}`}>{status}</span>
            {phase === "recording" ? (
              <small className="truncate text-[9px] text-[#858da1]">
                {activationModeRef.current === "hold" ? "松开完成" : "再次按下完成"}
              </small>
            ) : null}
          </div>
          <p
